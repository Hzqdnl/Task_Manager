...
useEffect(() => {
  ...
  return (
    <div>
        <p className='text-gray-600'>Assign Task To: </p>
        <Listbox
        value={selectedUsers}
        ...
        >
            <div className='relative mt-1'>
                <ListboxButton className='relative w-full cursor-default rounded bg-white pl-3 pr-10 
                text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm'>
                    <span
                    className='block truncate'
                    >{selectedUsers?.map((user) => user.name).join(', ')}</span>
                ...
                >
                    ...
                    py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm'>
                        {data?.map((user, index) => (
                            <ListboxOption
                            key={index}
                            className={({active}) => `relative cursor-default select-none py-2 pl-10 pr-4.5 ${active ? 'bg-yellow-100 text-yellow-900' : 'text-gray-900'}`}
                            value={user}
                            >
                                {({selected}) => (
                                    <>
